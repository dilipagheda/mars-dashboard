...
app.get('/apod', async (req, res) => {
    ...
    try {
        let response = await fetch(`https://api.nasa.gov/planetary/apod?api_key=${process.env.API_KEY}&date=${dateParam}`)
        console.log(`1 - ${response.status}`)
        if(response.status === 200) {
            let image = await response.json();
            res.send({ image });
        }else{
            let response = await fetch(`https://api.nasa.gov/planetary/apod?api_key=${process.env.API_KEY}&date=${yesterdayParam}`)
...
app.get('/photos', async (req, res) => {
    try {
        ...
        const manifestsResponse = await fetch(`https://api.nasa.gov/mars-photos/api/v1/manifests/${roverName}?api_key=${process.env.API_KEY}`)
            .then(res => res.json())
        
        const max_sol = manifestsResponse.photo_manifest.max_sol;

        const photosResponse = await fetch(`https://api.nasa.gov/mars-photos/api/v1/rovers/${roverName}/photos?sol=${max_sol}&api_key=${process.env.API_KEY}`)
            .then(res => res.json())
...
app.listen(port, () => console.log(`Mars Dashboard App is listening on port ${port}!`))