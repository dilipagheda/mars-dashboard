    ...
    const observerConfig = [
        {
            ...
            actionMethod: function(node){
                ...
                const marsRoverImagesButton = document.getElementById('mars-images-button')
        ...
        },
        {
            ...
            actionMethod: function(node) {
                ...
                curiosityButton.addEventListener('click', async () => {
                    const event = new CustomEvent('render', {detail: { rover: 'curiosity'}});
                    target.dispatchEvent(event)
                })
                opportunityButton.addEventListener('click', async () => {
                    const event = new CustomEvent('render', {detail: { rover: 'opportunity'}});
                    target.dispatchEvent(event)
                })
                spiritButton.addEventListener('click', async () => {
                    const event = new CustomEvent('render', {detail: { rover: 'spirit'}});
                    target.dispatchEvent(event)
                })
            }
        }        
